...
def set_mic_index(input_table):
    ...
    input_table[MIC_COLUMN_NAME] = input_table[1].str.find("MIC breakpoint")

    inside_mic = False

    # Search for MIC
    for idx in input_table.index:
        entry = input_table.at[idx, MIC_COLUMN_NAME]
        if entry == 0:  # Found the 'MIC breakpoint'
            inside_mic = True
            continue
        if inside_mic:
            if entry == -1:  # No 'MIC breakpoint'
                input_table.at[idx, MIC_COLUMN_NAME] = 1.0
            if pd.isna(
                input_table.at[idx, MIC_COLUMN_NAME]
...
def shrink_clean_table(input_table):
    ...
    # Shrink
    for idx in input_table.index:
        if input_table[MIC_COLUMN_NAME][idx] == 1.0:
            new_row = {
                "Name": input_table[input_table.columns[0]][idx],
                "S <=": input_table[input_table.columns[1]][idx],
                "R >": input_table[input_table.columns[2]][idx],
            }
            output_table = pd.concat(
                [output_table, pd.DataFrame([new_row])], ignore_index=True
            )

    # Clean table
    output_table = output_table.dropna(how="any")
    output_table = output_table[~output_table.isin(["None"]).any(axis=1)]
    output_table["Name"] = output_table["Name"].map(
        lambda x: re.sub(r",", "", x) if isinstance(x, str) else x
    )
    output_table["Name"] = output_table["Name"].map(
        lambda x: re.sub(r"\n", "", x) if isinstance(x, str) else x
    )
    output_table["S <="] = output_table["S <="].map(
        lambda x: re.sub(r"[()]", "", x) if isinstance(x, str) else x
    )
    output_table["R >"] = output_table["R >"].map(
        lambda x: re.sub(r"[()]", "", x) if isinstance(x, str) else x
    )
